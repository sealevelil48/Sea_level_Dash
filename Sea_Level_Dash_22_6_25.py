        ...
        if L is not None:
            try:
                ...
                if self.stations_cache is not None and \
                        (self.last_stations_fetch is None or datetime.now() - self.last_stations_fetch < timedelta(hours=1)):
            ...
            app.clientside_callback(
                ...
                function(n_clicks) {
                    ...
                    setTimeout(function () {
                        ...
                        const stationDropdown = document.querySelector('#station-dropdown .Select-value-label, #station-dropdown .Select__single-value');
                        const station = stationDropdown ? stationDropdown.textContent : 'sea_level_data';

                        const dateInputs = document.querySelectorAll('#date-range input');
                        const start = dateInputs[0] ? dateInputs[0].value : 'start';
                        const end = dateInputs[1] ? dateInputs[1].value : 'end';

                        const sanitize = str => str.replace(/[^a-zA-Z0-9_\\-]/g, "_");
                        const filename = `${sanitize(station)}_${sanitize(start)}_to_${sanitize(end)}.png`;
