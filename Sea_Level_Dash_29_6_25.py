...
class SeaLevelDashboard:
    ...
    def setup_api(self):
        ...
        def get_data():
            try:
                ...
                # New: Check if station is required for default data
        ...
        def get_station_map():
            try:
                df = self.fetch_station_locations()
                if df.empty:
                    return jsonify({'message': 'No station location data available'}), 404
    ...
    def get_live_data(self, station=None):
        ...
        try:
            # Get the most recent data for all stations
            latest_time_subq = (
                select([
                    L.c.Station,
                    ...
                    [
                        html.Div(
                            html.Img(
                                ...
                                style={"height": "200px", "position": "absolute", "left": "15px", "top": "15px"}
                            ),
                        ),
                        dbc.Col(
                            html.H1(
                                    ...
                                    children=[
                                        dbc.CardBody(
                                            ...
                                            children=[
                                                ...
                                                html.Div(
                                                    id="graph-container",
                                                    children=[dcc.Graph(id="line-graph", style={"height": "100%"},
    ...
    def register_optimized_callbacks(self, app):
            ...
            def handle_exports(table_clicks):
                ctx = callback_context
                if not ctx.triggered:
                    logging.warning("No trigger detected in export callback.")
                    raise PreventUpdate

                button_id = ctx.triggered[0]['prop_id'].split('.')[0]
                logging.info(f"Export button clicked: {button_id}")

                if button_id == "export-table-btn":
                    if not self.current_table_df_for_export.empty:
                        logging.info("Exporting table data...")
            ...
            def store_export_info(n_clicks, station, start_date, end_date):
                if n_clicks:
                    # Basic sanitization to prevent empty names
                    sanitized_station = station if station else "AllStations"
                    return {
                        'station': sanitized_station,
                        'start_date': start_date,
                        'end_date': end_date
                    }
                return no_update

            # Add Export-Filename-Store callback
            @app.callback(
            ...
            def generate_filename(export_info):
                ...
                try:
                    ...
                    sanitized_station = re.sub(r'[^\w\-]', '', export_info['station'])

                    # Use current date if no dates available
                    current_date = datetime.now().strftime('%Y-%m-%d')
                    start_date = export_info['start_date'] or current_date
                    end_date = export_info['end_date'] or current_date
                    ...
                    return f"{sanitized_station}_{sanitized_start}_to_{sanitized_end}.png"

                except Exception as e:
                    logging.error(f"Filename generation error: {e}")
                    return "sea_level_export.png"  # Default fallback

            # Combined Graph Export Clientside Callback
            app.clientside_callback(
                """
                function(n_clicks, export_filename) {  // Now we get the precomputed filename
                    ...
                    setTimeout(function () {
                        ...
                        Plotly.toImage(graphDiv, {
                            ...
                            a.href = url;
                            a.download = export_filename;  // Directly use the server-generated filename
                ...
                [Input("export-graph-btn", "n_clicks")],
                [State('export-filename-store', 'data')]  # Now using the filename store
            )
